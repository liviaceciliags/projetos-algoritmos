...
// Converte as sequências para maiúsculas
	for(int i = 0; i < t1; i++)
		s1[i] = toupper(s1[i]);
	for(int i = 0; i < t2; i++)
		s2[i] = toupper(s2[i]);
...
// Encontra a sequência comum
	vector<char> seq_comum;
	int i = 0, j = 0;
	while(i < t1 && j < t2)
	{
		if(s1[i] == s2[j])
		{
			seq_comum.push_back(s1[i]);
			i++;
			j++;
		}
...
// Exibe a sequência comum
for(auto c:seq_comum)
		cout << c << " ";
	cout << endl;
}